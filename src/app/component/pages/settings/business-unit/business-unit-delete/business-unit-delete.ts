...
@Component({
  selector: 'app-business-unit-delete',
  standalone: true,
  ...
  template: `
    ...
    <div class="modal-body">
      <p>คุณต้องการลบ "{{ item?.bu_name }}" ใช่หรือไม่?</p>
...
export class BusinessUnitDeleteComponent {
  @Input() item: any;
  loading = false;
  ...
  confirm(): void {
    this.loading = true;
    this.businessUnitService.deleteBusinessUnit(this.item.id).subscribe({
      next: () => {
