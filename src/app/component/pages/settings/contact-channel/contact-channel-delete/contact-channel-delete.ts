...
@Component({
  selector: 'app-contact-channel-delete',
  standalone: true,
  ...
  template: `
    ...
    <div class="modal-body">
      <p>คุณต้องการลบ "{{ item?.channel_name }}" ใช่หรือไม่?</p>
...
export class ContactChannelDeleteComponent {
  @Input() item: any;
  loading = false;
  ...
  confirm(): void {
    this.loading = true;
    this.contactChannelService.deleteContactChannel(this.item.id).subscribe({
      next: () => {
