...
@Component({
  ...
  template: `
    ...
    <div class="modal-body">
      <p>คุณต้องการลบ "{{ item?.om_name }}" ใช่หรือไม่?</p>
...
export class OmDeleteComponent {
  @Input() item: any;
  loading = false;
  ...
  confirm(): void {
    this.loading = true;
    this.omPersonsService.deleteOmPerson(this.item.id).subscribe({
      next: () => {
