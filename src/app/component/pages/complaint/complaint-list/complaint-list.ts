...
export class ComplaintListComponent implements OnInit, AfterViewInit {
  ...
  assigneeDropdownSettings: IDropdownSettings = {};
  ...
  ngAfterViewInit(): void {
    // Tags are now loaded from API in loadFilterOptions()
  }
  ...
  initDropdownSettings(): void {
    ...
    this.assigneeDropdownSettings = {
      ...baseSettings,
      idField: 'user_id',
      textField: 'first_name_last_name'
    };
  ...
  loadFilterOptions(): void {
    ...
    this.userService.getUserList().subscribe({
      next: (res: any) => {
        this.availableAssignees = Array.isArray(res) ? res : (res?.data || []);
      },
      error: () => {
    ...
    this.departmentService.getAll().subscribe({
      next: (res: any) => {
        ...
        // หา company ของ user จาก department
        const userDeptId = (this.user as any)?.department;
        if (userDeptId) {
          const userDept = this.availableDepartments.find((d: any) => String(d.id) === String(userDeptId));
          this.userCompany = userDept?.company || '';
        }
        this.applyFilters();
      },
      error: () => {
  ...
  loadData(): void {
    ...
    // Use optimized endpoint that includes sub-task data
    this.complaintService.getAllWithSubTasks().subscribe({
      next: (res: any) => {
        const data = Array.isArray(res) ? res : (res?.data || []);
        this.complaints = data.sort((a: any, b: any) => {
  ...
  applyFilters(): void {
    ...
    if (this.selectedAssignees.length > 0) {
      const assigneeId = this.selectedAssignees[0].user_id;
      filtered = filtered.filter(w =>
        (w.responsible_persons && Array.isArray(w.responsible_persons) &&
          w.responsible_persons.some((p: any) => p.user_id === assigneeId)) ||
        (w.subTaskAssignees && Array.isArray(w.subTaskAssignees) &&
          w.subTaskAssignees.some((p: any) => p.user_id === assigneeId))
      );
    }
    ...
    if (this.urgentFilter) {
      const isUrgent = this.urgentFilter === 'yes';
      filtered = filtered.filter(w =>
        w.subTasks?.some((t: any) => t.urgent === isUrgent)
      );
    }
