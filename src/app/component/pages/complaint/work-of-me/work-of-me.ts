...
@Component({
  selector: 'app-work-of-me',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, NgbPaginationModule, NgMultiSelectDropDownModule, DateThaiPipe],
  templateUrl: './work-of-me.html',
  styleUrl: './work-of-me.scss'
})
export class WorkOfMeComponent implements OnInit, AfterViewInit {
  user: any;
  complaints: any[] = [];
  filteredData: any[] = [];
  ...
  loadData(): void {
    ...
    this.complaintService.getAllWithSubTasks().subscribe({
      next: (res: any) => {
        ...
        // Filter by user who created the complaint
        this.complaints = data.filter((w: any) =>
          w.user_created?.user_id === this.user.user_id
        ).sort((a: any, b: any) => {
